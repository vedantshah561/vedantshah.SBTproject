...
# Files are in same folder as app.py (no artifacts subfolder)
BASE = os.path.dirname(__file__)

@st.cache_resource
def load_artifacts():
    model = joblib.load(os.path.join(BASE, 'carevision_model.pkl'))
    scaler = joblib.load(os.path.join(BASE, 'scaler.pkl'))
    encoders = joblib.load(os.path.join(BASE, 'encoders.pkl'))
    target_le = joblib.load(os.path.join(BASE, 'target_encoder.pkl'))
    feature_cols = joblib.load(os.path.join(BASE, 'feature_cols.pkl'))
    best_model_name = joblib.load(os.path.join(BASE, 'best_model_name.pkl'))
